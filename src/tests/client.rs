...
use smithay::reexports::wayland_protocols::wp::color_management::v1::client::wp_color_manager_v1::{
    Feature as ColorFeature, Primaries, RenderIntent, TransferFunction, WpColorManagerV1,
};
...
pub struct State {
    ...
    /// Features advertised by the color manager (from supported_feature events).
    pub color_features: Vec<ColorFeature>,
...
impl Client {
    ...
    /// Creates a parametric ExtLinear image description (Mesa WSI scRGB path) and attaches it.
    pub fn create_and_attach_ext_linear_description(
        &mut self,
        surface: &WlSurface,
        intent: RenderIntent,
        ...
        let creator = manager.create_parametric_creator(&self.qh, ());
        creator.set_tf_named(TransferFunction::ExtLinear);
        creator.set_primaries_named(Primaries::Srgb);
        if let Some(reference) = reference_white {
            // min (0.0001 cd/m² units), max, reference white in cd/m².
            creator.set_luminances(2, reference, reference);
        }
        let image = creator.create(&self.qh, ());
    ...
    /// Creates a Windows-scRGB image description and attaches it to a surface (Proton HDR path).
    pub fn create_and_attach_windows_scrgb(
        &mut self,
        surface: &WlSurface,
        intent: RenderIntent,
    ) {
...
impl Dispatch<WpColorManagerV1, ()> for State {
    fn event(
        ...
        use smithay::reexports::wayland_protocols::wp::color_management::v1::client::wp_color_manager_v1::Event;
        if let Event::SupportedFeature { feature } = event {
            if let Ok(feature) = feature.into_result() {
                state.color_features.push(feature);
            }
        }
    }
}

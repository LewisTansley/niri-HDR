...
#[test]
fn windows_scrgb_feature_is_advertised() {
    let mut f = fixture_with_hdr();
    f.add_output(1, (1920, 1080));
    ...
    use smithay::reexports::wayland_protocols::wp::color_management::v1::client::wp_color_manager_v1::Feature;
    assert!(
        f.client(id)
            .state
            .color_features
            .contains(&Feature::WindowsScrgb),
        "windows_scrgb feature must be advertised, got {:?}",
        f.client(id).state.color_features
    );
}

#[test]
fn extended_target_volume_feature_is_advertised() {
    // Mesa only offers VK_COLOR_SPACE_EXTENDED_SRGB_LINEAR_EXT when this feature is present.
    let mut f = fixture_with_hdr();
    f.add_output(1, (1920, 1080));
    ...
    assert!(
        ...
        "extended_target_volume feature must be advertised, got {:?}",
        f.client(id).state.color_features
    );
}

#[test]
fn windows_scrgb_on_subsurface_engages() {
    // Proton winewayland HDR often uses Windows-scRGB on a Vulkan subsurface.
    let mut f = fixture_with_hdr_mode_on();
    ...
    f.roundtrip(id);
    f.client(id)
        .create_and_attach_windows_scrgb(&subsurface, RenderIntent::Perceptual);
    f.roundtrip(id);
    let subsurface_clone = subsurface.clone();
    subsurface_clone.commit();
    f.client(id).window(&surface).surface.commit();
    ...
    let desc = f.niri().output_hdr_image_description(&output);
    assert!(
        desc.is_some_and(|d| d.is_windows_scrgb() && d.is_hdr() && !d.is_pq()),
        "Windows-scRGB on a subsurface must engage HDR (non-PQ), got {desc:?}"
    );
}

#[test]
fn ext_linear_on_subsurface_engages() {
    // Mesa Vulkan WSI tags EXTENDED_SRGB_LINEAR as parametric ExtLinear (Control / DXVK scRGB).
    let mut f = fixture_with_hdr_mode_on();
    ...
    let subsurface = f.client(id).create_committed_subsurface(&surface);
    f.roundtrip(id);
    f.client(id).create_and_attach_ext_linear_description(
        &subsurface,
        RenderIntent::Perceptual,
        Some(80),
    );
    ...
    assert!(
        ...
        "ExtLinear on a subsurface must engage HDR (non-PQ), got {desc:?}"
    );
}
